...
import { buildSimUnit, simulate, type CombatStats } from '../src/engine/simulate';
...
for (const id of ['ryan', 'taka', 'rachel']) {
  const unit = buildSimUnit(byId(id), content, s1[id], 1);
...
describe('SPEC v0.2 5.3 검증', () => {
  ...
  for (const id of ['ryan', 'taka']) {
    it(`2. 평균 예측: ${id} 세팅2 시뮬 평균이 실측 ±5% 이내`, () => {
      const unit = buildSimUnit(byId(id), content, s2[id], scales[id]);
  ...
  it('3. 흔들림: 라이언 세팅2 상대 표준편차가 9~36% 범위', () => {
    const unit = buildSimUnit(byId('ryan'), content, s2['ryan'], scales['ryan']);
    const res = simulate(unit, N, SEED);
  ...
  it('4. 결정성: 라이언 세팅1(치확·약확 100 이상)은 모든 판이 동일 (표준편차 0)', () => {
    const unit = buildSimUnit(byId('ryan'), content, s1['ryan'], scales['ryan']);
